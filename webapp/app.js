...
// ---------- зонд микрофона (03.09, временный) ----------
// Вопрос зонда: доступен ли MediaRecorder-путь в вебвью MAX (WKWebView/
// Android WebView отдают getUserMedia ТОЛЬКО если хост-приложение разрешило —
// из доков Bridge не видно, проверяется только живой пробой). SpeechRecognition
// в вебвью обеих сред не экспонируется — зонд это подтвердит/опровергнет.
// Доступ: 5 тапов по номеру версии на экране настройки — пользователям не виден.
// УДАЛИТЬ после решения по голосовому вводу.
let probeTaps = 0;
let probeTapTimer = null;
document.getElementById("app-version").addEventListener("click", () => {
  probeTaps += 1;
  clearTimeout(probeTapTimer);
  probeTapTimer = setTimeout(() => { probeTaps = 0; }, 1500);
  if (probeTaps >= 5) { probeTaps = 0; showMicProbe(); }
});

function showMicProbe() {
  let panel = document.getElementById("mic-probe");
  if (!panel) {
    panel = document.createElement("div");
    panel.id = "mic-probe";
    panel.style.cssText =
      "position:fixed;inset:auto 12px 12px;z-index:99;background:#fff;border:1.5px solid #185fa5;" +
      "border-radius:14px;padding:12px 14px;font:13px/1.5 -apple-system,sans-serif;color:#2c2c2a;" +
      "box-shadow:0 6px 24px rgba(0,0,0,.18);max-height:60vh;overflow-y:auto;";
    document.body.appendChild(panel);
  }
  const lines = [];
  const put = (k, v) => lines.push(`<b>${k}</b>: ${v}`);
  put("платформа", `${window.WebApp?.platform ?? "?"} · ${navigator.userAgent.slice(0, 60)}…`);
  put("secureContext", String(window.isSecureContext));
  put("mediaDevices", String(Boolean(navigator.mediaDevices)));
  put("getUserMedia", String(Boolean(navigator.mediaDevices?.getUserMedia)));
  put("MediaRecorder", String(typeof MediaRecorder !== "undefined"));
  put("SpeechRecognition", String(Boolean(window.SpeechRecognition || window.webkitSpeechRecognition)));
  if (typeof MediaRecorder !== "undefined") {
    const mimes = ["audio/webm", "audio/mp4", "audio/ogg", "audio/wav"]
      .filter((m) => MediaRecorder.isTypeSupported?.(m));
    put("форматы записи", mimes.join(", ") || "ни один из типовых");
  }
  panel.innerHTML =
    `<p style="margin:0 0 8px;font-weight:600">Зонд микрофона</p>` +
...
async function runMicProbe() {
  ...
  if (!navigator.mediaDevices?.getUserMedia) { say("getUserMedia недоступен — путь закрыт"); return; }
  say("запрашиваю доступ к микрофону…");
  try {
    const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
    if (typeof MediaRecorder === "undefined") {
      stream.getTracks().forEach((t) => t.stop());
      say("доступ дали, но MediaRecorder отсутствует");
      return;
    }
    ...
    rec.onstop = () => {
      ...
      say(`✅ ЗАПИСЬ РАБОТАЕТ: ${blob.size} байт, тип ${rec.mimeType || "(пустой)"}`);
    };
    rec.start();
    say("пишу 2 секунды — скажи что-нибудь…");
    setTimeout(() => rec.stop(), 2000);
  } catch (err) {
    say(`❌ отказ: ${err.name} — ${err.message}`);
  }
}
