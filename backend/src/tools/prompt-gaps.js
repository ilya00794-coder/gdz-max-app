...
const SUBJECT_FOCUS = {
  ...
  "ОБЗР":
    "ОСОБОЕ ВНИМАНИЕ, безопасность: может ли какая-либо рекомендация решения НАВРЕДИТЬ при " +
    "БУКВАЛЬНОМ исполнении ребёнком? Примеры вреда: неверный порядок действий первой помощи, " +
    "устаревшие приёмы (запрокинуть голову при носовом кровотечении, масло/лёд на ожог), " +
    "призыв самому тушить огонь или спасать из воды там, где ребёнку положено звать взрослых " +
    "и звонить 112. Каждую потенциально вредную рекомендацию фиксируй в missingForAge " +
    "ПЕРВОЙ строкой, начиная со слова ОПАСНО.",
  "Обществознание":
    "ОСОБОЕ ВНИМАНИЕ, право несовершеннолетних: если решение касается прав, обязанностей, " +
    "конфликта с законом или трудовых отношений подростка — может ли совет НАВРЕДИТЬ при " +
    "буквальном исполнении (спорить или сопротивляться полиции, подписывать документы без " +
    "законного представителя, соглашаться на неоформленную работу, скрывать произошедшее " +
    "от родителей)? Неверно названные нормы (возраст, часы работы, права при задержании) " +
    "фиксируй как фактическую ошибку; вредные советы — ПЕРВОЙ строкой, начиная со слова ОПАСНО.",
  "Окружающий мир":
...
ФИНАЛЬНЫЙ ОТВЕТ РЕШЕНИЯ: ${solution.finalAnswer}${
  ...
  solution.figure
    ? `\nК РЕШЕНИЮ ПРИЛОЖЕН РИСУНОК (${{ circles: "кружки для счёта", numberline: "числовой луч", rectangle: "чертёж прямоугольника с подписанными сторонами", square: "чертёж квадрата с подписанной стороной", "adjacent-angles": "чертёж смежных углов с дугами и значениями", "vertical-angles": "чертёж вертикальных углов с дугами и значениями", "parallel-lines": "чертёж параллельных прямых с секущей и отмеченной парой углов", triangle: "чертёж треугольника с подписанными вершинами, углами/сторонами и элементами", parallelogram: "чертёж параллелограмма с вершинами, углами и диагоналями", rhombus: "чертёж ромба с диагоналями и точкой пересечения", trapezoid: "чертёж трапеции с основаниями и средней линией" }[solution.figure.kind] ?? solution.figure.kind}) — приложение рисует его ученику рядом с шагами.`
    : ""
...
async function main() {
  ...
  const judged = await pool(
    ...
    async (job) => {
      ...
      // Для НЕвычислимых предметов sympy обязан быть null: пустая строка ""
      // в формате набора значит «корней нет» и для музыки/истории — авторская
      // ошибка, которая молча роняет весь счётчик (случилось на наборе gnp).
