...
// ВЕРСИЯ НОРМАЛИЗАЦИИ. При ЛЮБОЙ правке normalizeTaskText ниже — БАМПНУТЬ
// (v3 → v4 …). Иначе старые кэш-записи будут находиться по НОВОЙ логике
// нормализации, а решались по старой. Версия входит в ключ, поэтому после
// бампа старые записи просто перестают находиться (чистый промах →
// перерешивание), а не превращаются в мусор с неверным содержимым.
// Это ЕДИНСТВЕННОЕ, что надо помнить при правке нормализации.
const NORM_VERSION = "v3";
...
export function buildCacheKey({ grade, subject, rawText }) {
  ...
  return `text:${NORM_VERSION}:${grade}:${subject}:${hash}`.toLowerCase();
}
