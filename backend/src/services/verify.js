...
export function parseCandidateAnswer(candidateAnswer) {
  ...
  // LaTeX-ОБЁРТКИ vision снимаем как класс (п.6): vision пишет ответы в
  // LaTeX-настроении ($8\frac{1}{6}$, \dfrac, \text{ кг}, распорки \, \qquad),
  // и обёртка ломала грамматику литерала → ложный FALSE на верном ответе.
  // Снимается только ОБЁРТКА — грамматика ответа не расширяется: интервалы,
  // корни и выражения как были unsupported, так и остаются.
  const text = normalizeMathText(candidateAnswer)
    .trim()
    .replace(/^ответ\s*[:.]?\s*/i, "")
    .replace(/\$/g, "") // доллары — всегда обёртка, содержимым не бывают
    .replace(/\\dfrac/g, "\\frac")
    .replace(/\\text\{([^{}]*)\}/g, " $1 ") // \text{ кг} → « кг » (кириллица уйдёт ниже)
    .replace(/\\[,;!]|\\q?quad/g, " ") // распорки \, \; \! \quad \qquad
    // \frac{a}{b} → « a/b»: смешанные дроби приходят и как «8 17/51» (почерк),
    // и как «8\frac{17}{51}» (LaTeX); знак числителя — внутри скобок: \frac{-1}{2}.
