...
- kind "any" — ОДНО И ТО ЖЕ число, записанное в нескольких формах;
  ...
  физического пересчёта. РАЗНЫЕ ЕДИНИЦЫ ИЗМЕРЕНИЯ В "any" ЗАПРЕЩЕНЫ:
  пересчёт между единицами — это вычисление, в котором можно ошибиться,
  а "any" засчитал бы ответ по любой форме — и неверный пересчёт прошёл
  бы проверку незамеченным. «1 кВт·ч (это 3,6 МДж)» → kind "all",
  values: [{value:"1", unit:"кВт·ч"}] — одна выбранная единица; вторая
  форма остаётся только в тексте решения для человека.
  Все элементы "any" ОБЯЗАНЫ быть равны между собой — это проверяется
  автоматически, неравные формы в "any" — твоя ошибка.
