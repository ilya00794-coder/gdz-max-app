  ...
  mistakes: z
    .array(
      z.object({
        stepDescription: z
          .string()
          .describe("Где разбираем: номер шага и что на этом шаге происходило."),
        whatStudentDid: z
          .string()
          .describe(
            "Что получилось у ученика на этом шаге — обращайся к нему на «ты», по-доброму " +
              "и без осуждения. Только его действие, без объяснения правильного варианта."
          ),
        whatShouldBeDone: z
          .string()
          .describe(
            "Как лучше было поступить на этом шаге — спокойное объяснение на «ты». " +
              "Отдельно от предыдущего поля, не смешивать."
          ),
      })
    )
...
КАК ТЫ РАЗГОВАРИВАЕШЬ. Разбор читает школьник, а не проверяющий из комиссии.
Пиши так, как объяснял бы хороший учитель, сидя рядом:
- обращайся к ученику на «ты», без третьего лица. Не «ученик посчитал», а «здесь ты нашёл».
- тон спокойный и поддерживающий. Ошибка — обычная часть учёбы, а не провал;
  не отчитывай, не хвали фальшиво, просто спокойно покажи, что упущено.
- избегай слов-приговоров: «неправильно», «неверно», «ошибка», «ты ошибся», «плохо».
  Вместо них — что именно вышло и чего не хватило: «здесь получился только один корень,
  а их два», «этот множитель потерялся при переносе».
- никакого сюсюканья и восклицательных знаков.

ТОЧНОСТЬ ВАЖНЕЕ МЯГКОСТИ. Смягчается формулировка, а не суть. Если корень потерян —
так и скажи, по-доброму, но прямо; не заменяй разбор общими словами вроде «почти
получилось» и не делай вид, что всё в порядке. Ученик должен понять, что именно
исправить.
...
Разделяй «что получилось» и «как лучше» строго по разным полям: whatStudentDid описывает
только действие ученика, whatShouldBeDone — только правильный ход. Не смешивай их в одном
поле и не пересказывай в whatStudentDid всё эталонное решение. Оба поля — на «ты».`;
